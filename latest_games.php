...
function fetchLatestGames(int $page = 1): array {
    $sourceUrl = 'https://' . SOURCE_DOMAIN . '/new-games';
    ...
    if ($curlError || $httpCode !== 200 || empty($response)) {
        return ['apps' => [], 'error' => "Error fetching latest games: " . ($curlError ?: "HTTP " . $httpCode)];
    }
    ...
    try {
        ...
        $appNodes = $xpath->query("//div[contains(@class, 'list')]");
...
$result = fetchLatestGames($currentPage);
$apps = $result['apps'] ?? [];
...
<div class="max-w-screen-xl mx-auto px-4 py-8">
    ...
    <?php elseif (empty($apps) && $currentPage === 1): ?>
        <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4" role="alert">
            <p>No recently updated games were found.</p>
        </div>
    <?php else: ?>
